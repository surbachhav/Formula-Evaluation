...
class VariableExpressionTest {
    ...
    void testEvalBound() throws UnboundVariableException {
        ...
        assertEquals(2.0, expr.eval(vars));
    }
...
class OperationExpressionTest {
    ...
    void testEvalAddBound() throws UnboundVariableException {
        ...
        Expression expr = new Operation(Operator.ADD, new Constant(1.5), x);
        assertEquals(3.5, expr.eval(vars));

        Expression exprTwo = new Operation(Operator.ADD, y, x);
        assertEquals(3.5, exprTwo.eval(vars));
    }
    ...
    void testInfixLeaves() {
        Expression expr = new Operation(Operator.ADD, new Variable("x"), new Constant(2.0));
    ...
    void testOptimizeAdd() {
        Expression expr = new Operation(Operator.ADD, new Constant(1.5), new Constant(2.0));
        Expression answer = new Constant(3.5);
        assertEquals(answer, expr.optimize(MapVarTable.empty()));

        //An operation node for ADD with one Variable and one Expression with two Constants as leaves
        // should optimize to an operation node for ADD with the Variable and one Constant.
        expr = new Operation(Operator.ADD, new Variable("x"), new Operation(Operator.MULTIPLY, new Constant(2.0), new Constant(3.0)));
        answer = new Operation(Operator.ADD, new Variable("x"), new Constant(6.0));
        assertEquals(answer, expr.optimize(MapVarTable.empty()));
    }
}
...
class ConditionalExpressionTest {
    ...
    void testEvalCondBound() throws UnboundVariableException {
        ...
        assertEquals(5.0, expr.eval(MapVarTable.of("x", 0)));
    }
    ...
    void testOptimizeConstCondition() {
        ...
        Expression expr = new Conditional(
            new Constant(1.0),
            new Operation(Operator.ADD, new Variable("x"), new Constant(5.0)),
            new Constant(1.0)
        );

        Expression answer = new Constant(7.0);
        assertEquals(answer, expr.optimize(vars));
    }
    ...
    void testOptimizeExprCondition() {
        ...
        Expression expr = new Conditional(
                new Operation(Operator.SUBTRACT, new Variable("x"), new Constant(1.0)),
                new Operation(Operator.ADD, new Constant(1.0), new Constant(1.0)),
                new Conditional(
                        new Constant(1.0),
                        new Constant(2.0),
                        new Constant(3.0)
        ...
        Expression answer = new Constant(2.0);
        assertEquals(answer, expr.optimize(vars));
    }
}
