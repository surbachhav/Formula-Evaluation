...
public class CsvEvaluator {
    ...
    public static void evaluateCsv(CSVParser parser, CSVPrinter printer) throws IOException {
        ...
        for (CSVRecord record : parser){
            ...
            for (int col = 0; col < record.size(); col++) {
                ...
                if (record.get(col).startsWith("=")) {
                    String withoutEquals = record.get(col).substring(1);
                    try {
                        Expression expr = RpnParser.parse(withoutEquals, defs);
                        String cellLabel = colToLetters(col + 1) + row;
                        vars.set(cellLabel, expr.eval(vars));
                        printer.print(String.valueOf(expr.eval(vars)));
                    }
                    catch (Exception e){
                        printer.print("#N/A");
                    }
    ...
    public static String colToLetters(int n) {
        ...
        if (q == 0) {
            ...
            return colToLetters(q) + (char)('A' + r);
        }
    }
